...
class UserNameValidation(View):
    def post(self, request):
        ...
        if User.objects.filter(username=username).exists():
            return JsonResponse({'username_error': 'Username already in use, Choose another one'}
                                , status=409)
...
class EmailValidation(View):
    def post(self, request):
        ...
        if not validate_email(email):
            return JsonResponse({'email_error': 'Email is not valid'}
                                , status=400)
        if User.objects.filter(email=email).exists():
            return JsonResponse({'email_error': 'Email already in use, Please choose another one'}
                                , status=409)
    ...
    def get(self, request, uidb64, token):
        ...
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                ...
                return render(request, 'authentication/reset_password.html')
            messages.success(request, "Password reset successfully, you an login with the new password")
            return redirect('login')
        except Exception as identifier:
            pass
